...
queryString = """WITH missing AS(select
	t1.matchid,
	t1.playerid,
	t1.teamid
from mapstats t1
left outer join players t2
on t1.playerid=t2.id
where t2.id is null)
SELECT
	DISTINCT ON (missing.playerid) playerid,
	missing.matchid
FROM
	missing
ORDER BY
...
async def main():
	...
	missing = await db.fetch(queryString)

	conn = await db.acquire()
	async with conn.transaction():
		for bogey in missing:
			response = requests.get(f"https://api.overwatchleague.com/matches/{bogey['matchid']}")
			games = response.json()["games"]
			
			playerMatch = False
			for game in games:
				for player in game["players"]:
					if player["player"]["id"] == bogey["playerid"]:
						...
						await conn.execute("INSERT INTO players VALUES ($1, $2, $3, $4, $5, $6, $7, $8);", player["player"]["id"], player["player"]["name"], f"{player['player']['givenName']} {player['player']['familyName']}", nationality, player["player"]["homeLocation"], player["player"]["attributes"]["player_number"], player["player"]["attributes"]["role"], player["team"]["id"])
